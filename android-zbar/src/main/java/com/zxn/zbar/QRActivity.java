...
import com.zxn.zbar.Qr.Symbol;
import com.zxn.zbar.utils.GetPathFromUri;
import com.zxn.zbar.utils.QRUtils;
import com.zxn.zbar.view.ScanView;
import com.zxn.zbar.view.VerticalSeekBar;
...
public class QRActivity extends Activity implements View.OnClickListener {
    ...
    private TextView tv_input_imei_code;
    ...
    private void initView() {
        ...
        tv_top_text = (TextView) findViewById(R.id.tv_top_text);
        tv_input_imei_code = (TextView) findViewById(R.id.tv_input_imei_code);

        tv_input_imei_code.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (QrManager.getInstance().getResultCallback() instanceof QrManager.OnScanStopCallback) {
                    QrManager.OnScanStopCallback callback = (QrManager.OnScanStopCallback) QrManager.getInstance().getResultCallback();
                    callback.onStopScan();
                }
                finish();
            }
        ...
        tv_top_text.setVisibility(options.isShow_des() ? View.VISIBLE : View.GONE);
        tv_input_imei_code.setVisibility(options.isShowInputCode() ? View.VISIBLE : View.GONE);
        vsb_zoom.setVisibility(options.isShow_zoom() ? View.VISIBLE : View.GONE);

        tv_des.setText(options.getDes_text());
        tv_top_text.setText(options.getTopText());
